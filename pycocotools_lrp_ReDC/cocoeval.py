...
from .utils import calculateCAR
...
class COCOeval:
    ...
    def evaluate(self, eval_cal = False):
        '''
        Run per image evaluation on given images and store results
         (a list of dict) in self.evalImgs
        :return: None
        '''
        self.eval_cal = eval_cal
        tic = time.time()
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        dtCAR = np.zeros((T, D, 4))
        dtSign = np.zeros((D, 4))
        dtBestIoU = np.zeros((D,), dtype=np.float32)

        if len(gt) > 0 and len(ious) > 0:
            for dind, d in enumerate(dt):
                best_gt = np.argmax(ious[dind])
                dtBestIoU[dind] = ious[dind, best_gt]   # 추가

                gt_box = gt[best_gt]['bbox']
                dt_box = d['bbox']

                dx1 = dt_box[0] - gt_box[0]
                dy1 = dt_box[1] - gt_box[1]
                dx2 = (dt_box[0] + dt_box[2]) - (gt_box[0] + gt_box[2])
                dy2 = (dt_box[1] + dt_box[3]) - (gt_box[1] + gt_box[3])

                dtSign[dind] = np.sign([dx1, dy1, dx2, dy2])
        else:
            dtSign[:] = 0
            dtBestIoU[:] = 0
        ...
        if not len(ious) == 0:
            for tind, t in enumerate(p.iouThrs):
                for dind, d in enumerate(dt):
                    ...
                    dt_box = d['bbox']
                    gt_box = gt[m]['bbox']
                    dtCAR[tind, dind] = calculateCAR(dt_box, gt_box)
        ...
        det_scores = [d['score'] for d in dt]
        dtfeature = [d['bbox_feature'] for d in dt]
        dtlogits = [d['all_logits'] for d in dt]
        dt_bbox = np.asarray([d['bbox'] for d in dt]).reshape(-1, 4)
        dt_imgid = [d['image_id'] for d in dt]
        ...
        if self.eval_cal:
            result.update({
                'cw_score': [d['cw_score'] for d in dt if 'cw_score' in d],
                'pred_sign': [d['pred_sign'] for d in dt if 'pred_sign' in d],
            })
